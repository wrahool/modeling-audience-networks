...
get_simulated_network <- function(n1, n2, n3, n4, tau) {
  outlet_ids <- 1:n1
  p_ids <- 1:n2
  types <- LETTERS[1:n4]
  ...
  for(p in 1:1000) {
    ...
    selective_choices_allowed <- n3 - tau * n3
...
set.seed(42)
simulation_tbl <- NULL
for(tau in seq(0.8, 0.9, by = 0.001)) {
  message(tau)
  g <- get_simulated_network(n1, n2, n3, n4, tau)
  plot.igraph(g)
  walktrap.community(g)
  
  simulation_tbl <- tibble(tau = tau,
